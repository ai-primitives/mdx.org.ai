...
async function generateTypeDefinitions(files: ValidMDXParseResult[]): Promise<string> {
  ...
  $type: '${metadata.$type}';
        ...
        console.error(`Error generating interface for ${metadata.$type}:`, error);
        return '';
      }
    };
    ...
    const interfaces = files.reduce((acc: string[], { metadata }) => {
      try {
        ...
        if (!typeStr) {
          console.warn(`Failed to generate interface for ${type}`);
          return acc;
        }
        ...
        console.error(`Error in interface generation for ${metadata.$type}:`, error);
        return acc;
      }
    }, []);

    if (interfaces.length === 0) {
      console.warn('No valid interfaces generated');
      return '';
    }
  ...
  ${properties.join('\n  ')}
    ...
    const output = `// Generated by generate-types.ts
${baseInterface}
...
`;
    ...
    if (!output.trim()) {
      throw new Error('Generated type definitions are empty');
    }
  ...
  try {
    ...
    const contentDirs = await Promise.all([
      ...
      try {
        ...
        log(`Checking directory: ${absolutePath}`);

        try {
          await fsPromises.access(absolutePath, fsPromises.constants.R_OK);
          log(`Directory exists and is readable: ${absolutePath}`);
          return absolutePath;
        } catch (error) {
          console.warn(`Directory not accessible: ${absolutePath}`, error);
          return null;
        }
      } catch (error) {
        console.error(`Error resolving directory ${dir}:`, error);
        return null;
      }
    ...
    const mdxFilesPromises = (contentDirs.filter((dir): dir is string => dir !== null))
